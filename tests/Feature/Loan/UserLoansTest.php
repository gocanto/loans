...
class UserLoansTest extends TestCase
{
    ...
    {
        $response = $this->get(\route('loans.user.loan', ['uuid' => 'foo']));
    ...
    public function authorizedUsersAreAllowed(): void
    {
        ...
        $response = $this->get(\route('loans.user.loan', ['uuid' => $loan->user->uuid]), [
        ...
        //none existing users
        $response = $this->get(\route('loans.user.loan', ['uuid' => 'foo-bar-biz']), [
            'X-API-Key' => \config('loans.admin_token')
        ]);
